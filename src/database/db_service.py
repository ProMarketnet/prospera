...
class DatabaseService:
    ...
    def get_analytics_data(self, profile_id: int) -> Dict[str, Any]:
    ...
    def list_business_profiles(self) -> List[Dict[str, Any]]:
        """List all business profiles"""
        if not self.db_available:
            logger.info("Database not available - no stored profiles to list")
            return []
        
        return []
    
    def get_latest_analysis(self, profile_id: int) -> Optional[Dict[str, Any]]:
        """Get latest analysis for profile"""
        if not self.db_available:
            return None
